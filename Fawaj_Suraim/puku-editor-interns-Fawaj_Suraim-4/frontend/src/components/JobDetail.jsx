...
export default function JobDetail({ job, onCancelDone, onResubmit, onDeleted, authLocked = false }) {
  ...
  const onCancel = async () => {
    ...
    try {
      ...
      setCancelling(false);
      // Drop the rerun intent too — otherwise the next time the user clicks
      // Cancel (on the same or different job), the rerun countdown effect
      // would fire on the stale image+command. Only the failing cancel
      // cleared `cancelling`; the user must hit Cancel & Re-run again to
      // opt back into resubmitting.
      setPendingRerun(null);
      return;
    }
