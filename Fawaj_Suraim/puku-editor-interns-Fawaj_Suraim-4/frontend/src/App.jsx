...
import { listJobs, deleteJobs, submitJob, hasApiKey } from './api';
...
export default function App() {
  ...
  // True when no API key is available from any source (localStorage OR
  // VITE_API_KEY baked into the bundle). The auth-locked banner appears
  // only after the first 401 round-trip; this one shows up immediately so
  // the user knows to paste the key before any request goes out. Mostly
  // matters in Docker setups where the build-time key wasn't forwarded
  // (multi-tenant prod, misconfigured .env, etc.).
  ...
  useEffect(() => {
    ...
    // Compute once on mount — localStorage doesn't change without a user
    // action, and the Header calls onApiKeySaved() to refresh this flag
    // when the user types+saves a key.
  ...
  const onApiKeySaved = async () => {
    setAuthLocked(false);
    // Recompute on every save — typing the key clears the missing-key
    // banner immediately, and clearing the key re-shows it.
    setKeyMissing(!hasApiKey());
    try {
      await hydrate();
    } catch {
  ...
  return (
    ...
    <div className="relative min-h-screen w-full overflow-x-hidden text-slate-100">
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        />
        {keyMissing && !authLocked && (
          // Fires only when neither localStorage nor VITE_API_KEY has a
          // key. We deliberately hide this once authLocked is also true —
          // at that point the amber 'API key rejected' banner is the
          // more useful signal (the user clearly has the field filled in
          // with the wrong value, not an empty one).
          <div
            role="status"
            className="mx-3 mt-3 rounded-md bg-amber-500/10 px-3 py-2 text-xs text-amber-300 ring-1 ring-amber-500/30"
          >
            No API key configured — paste one into the header field and press Save (or Enter).
          </div>
        )}
        {authLocked && (
          <div
